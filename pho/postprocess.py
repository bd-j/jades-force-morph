...
from forcepho.reconstruction import Residuals, Samples
from forcepho.superscene import isophotal_radius

from utils import cat_to_reg, max_roi


__all__ = ["write_images", "write_sourcereg", "write_patchreg",
...
def compare_final(root="opt_all_v0.5_full"):
    from forcepho.utils import read_config, rectify_catalog

    samples = Samples(f"{root}/patches/patch0_samples.h5")
    config = read_config(samples.config)

    #det = fits.getdata(config["detection_catalog"])
    raw = fits.getdata(config["raw_catalog"])
    init, bands, hdr = rectify_catalog(config["raw_catalog"])
    final = fits.getdata(f"{root}/{config['scene_catalog']}")
